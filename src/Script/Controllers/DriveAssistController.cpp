...
void DriveAssistController::ApplySelectiveBrake(
    Vehicle veh, float severity, float yawError, bool oversteer) {
    if (severity <= 0.01f)
        return;

    Vector3 minimum{};
    Vector3 maximum{};
    MISC::GET_MODEL_DIMENSIONS(ENTITY::GET_ENTITY_MODEL(veh),
                               &minimum, &maximum);
    ...
    ENTITY::APPLY_FORCE_TO_ENTITY(
        veh, 1, 0.0f, -brakingForce, 0.0f,
        offsetX, offsetY, minimum.z + 0.12f,
        0, TRUE, TRUE, TRUE, FALSE, TRUE);
}

void DriveAssistController::Update(
    Vehicle veh, VehicleData& data, int gear, float clutchDisengagement,
    float& throttle, float& brake, float forwardSpeed, bool engineOn,
    bool automaticMode) {
    if (veh != m_vehicle) {
        Reset();
        m_vehicle = veh;
    ...
    TractionControl::Update(veh, data, forwardSpeed, gear,
                            clutchDisengagement, throttle);
    brake = BrakeSystem::UpdateABS(
        veh, data, brake, forwardSpeed, gear < 0);
    ...
    const float desiredYaw =
        std::clamp(std::tan(steering * 0.48f) *
                       absoluteForwardSpeed / wheelbase,
                   -1.8f, 1.8f);
    ...
    const float yawThreshold =
        0.18f + 0.012f * absoluteForwardSpeed;
    const float yawSeverity =
        Clamp01((std::fabs(yawError) - yawThreshold) / 1.20f);
    const float lateralG =
        std::fabs(m_filteredLateralAcceleration) / 9.80665f;
    const float lateralSeverity =
        Clamp01((lateralG - 0.38f) / 0.72f);
    const bool escAllowed =
        Config::EscEnabled && engineOn && gear != 0 &&
        absoluteForwardSpeed * 3.6f >=
            std::clamp(Config::EscMinSpeedKmH, 5.0f, 80.0f) &&
        !ENTITY::IS_ENTITY_IN_AIR(veh);
    const float requestedEsc =
        escAllowed
            ? std::max(slipSeverity,
                       yawSeverity * (0.45f + lateralSeverity * 0.55f))
            : 0.0f;
    m_state.stabilityError = SmoothToward(
        m_state.stabilityError, requestedEsc,
    ...
    if (m_state.escActive) {
        ...
        const bool oversteer = std::fabs(angularVelocity.z) >
                              std::fabs(desiredYaw);
    ...
    m_state.rollWarning =
        Config::RolloverAssist && absoluteForwardSpeed > 3.0f &&
        !ENTITY::IS_ENTITY_IN_AIR(veh) && rollAngle >= rollThreshold;
    if (m_state.rollWarning) {
        const float rollSeverity =
    ...
    if (m_state.escActive && !m_escWasActive) {
        ...
        DrivingEventBus::Publish(
            DrivingEventBus::Event::ESCActivated, eventData);
    }
    ...
    if (m_state.rollWarning && !m_rollWasActive) {
        ...
        DrivingEventBus::Publish(
            DrivingEventBus::Event::RolloverWarning, eventData);
        LOG_WARN(Physics, "Rollover warning: roll=%.1f speed=%.1fkm/h",
                 rollAngle, absoluteForwardSpeed * 3.6f);
    }
