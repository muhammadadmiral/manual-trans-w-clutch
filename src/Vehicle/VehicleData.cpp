...
bool IsWritableAddress(uintptr_t address, size_t size) {
  ...
  if (info.State != MEM_COMMIT || !IsWritableProtection(info.Protect)) return false;
...
uint32_t ReadHexOffset(const char *iniPath, const char *section, const char *key) {
  char buffer[32]{};
  const DWORD count = GetPrivateProfileStringA(section, key, "", buffer, sizeof(buffer), iniPath);
  if (count == 0) return 0;

  char *end = nullptr;
  const unsigned long value = std::strtoul(buffer, &end, 0);
  if (end == buffer || *end != '\0' || value > (std::numeric_limits<uint32_t>::max)()) {
    return 0;
  }
...
bool GetModuleFileVersion(HMODULE module, std::string &out) {
  ...
  UINT infoSize = 0;
  if (!VerQueryValueA(buffer.data(), "\\", reinterpret_cast<void **>(&info), &infoSize) || info == nullptr) {
    return false;
  }
...
std::string VehicleData::GetGameBuildVersion() {
  std::string version;
  if (!GetModuleFileVersion(GetModuleHandleW(nullptr), version)) return "";
  return version;
}

bool VehicleData::AreOffsetsSane(const VehicleOffsets &value) {
  if (!value.IsCompleteCore()) return false;

  constexpr uint32_t kMinimumOffset = 0x100;
  constexpr uint32_t kMaximumOffset = 0x8000;
  const uint32_t fields[] = {value.Gear, value.NextGear, value.Clutch, value.RPM};

  for (const uint32_t field : fields) {
    if (field < kMinimumOffset || field >= kMaximumOffset) return false;
  }

  const uint32_t gearDistance = value.Gear > value.NextGear ? value.Gear - value.NextGear : value.NextGear - value.Gear;
  if (value.Gear == value.NextGear || gearDistance > 0x20) return false;
  
  const uint32_t driveDistance = value.Clutch > value.RPM ? value.Clutch - value.RPM : value.RPM - value.Clutch;
...
bool VehicleData::ResolveOffsetsByPattern(VehicleOffsets &result) {
  ...
  if (!AreOffsetsSane(result)) {
    lastFailureReason = "pattern offsets failed sanity check (game build outdated)";
    return false;
  }
  return true;
}
...
bool VehicleData::LoadOffsetsFromIni(HMODULE pluginModule, VehicleOffsets &result) {
  ...
  if (!buildVersion.empty()) {
    ...
    versioned.NextGear = ReadHexOffset(iniPath, versionedSection.c_str(), "NextGear");
    versioned.Clutch = ReadHexOffset(iniPath, versionedSection.c_str(), "Clutch");
    versioned.RPM = ReadHexOffset(iniPath, versionedSection.c_str(), "RPM");
    versioned.TopGear = ReadHexOffset(iniPath, versionedSection.c_str(), "TopGear");
    versioned.DriveForce = ReadHexOffset(iniPath, versionedSection.c_str(), "DriveForce");
    versioned.FuelLevel = ReadHexOffset(iniPath, versionedSection.c_str(), "FuelLevel");
    versioned.LightsBroken = ReadHexOffset(iniPath, versionedSection.c_str(), "LightsBroken");
    versioned.LightsVisuallyBroken = ReadHexOffset(iniPath, versionedSection.c_str(), "LightsVisuallyBroken");
    versioned.HoverTransformRatioLerp = ReadHexOffset(iniPath, versionedSection.c_str(), "HoverTransformRatioLerp");
    versioned.GearRatios = ReadHexOffset(iniPath, versionedSection.c_str(), "GearRatios");
  ...
  candidate.LightsVisuallyBroken = ReadHexOffset(iniPath, "Offsets", "LightsVisuallyBroken");
  candidate.HoverTransformRatioLerp = ReadHexOffset(iniPath, "Offsets", "HoverTransformRatioLerp");
...
void VehicleData::SaveOffsetsToIni(HMODULE pluginModule, const VehicleOffsets &offsets) {
  char iniPath[MAX_PATH]{};
  if (!BuildIniPath(pluginModule, iniPath)) return;
...
void VehicleData::UpdateCalibration(HMODULE pluginModule, int vehicleHandle, bool isEngineOn, bool isRevving) {
  if (initialized || calibState == CalibrationState::None ||
      calibState == CalibrationState::Done || calibState == CalibrationState::Failed)
    return;
  ...
  if (calibState == CalibrationState::WaitingForEngineOff) {
    if (!isEngineOn) calibState = CalibrationState::ScanningEngineOff;
    ...
    for (uint32_t offset = 0x600; offset < 0xC00; offset += 4) {
      if (AOBScanner::IsReadable(calibVehicle.GetAddress() + offset, 4)) {
        ...
        if (val == 0.0f) candidateOffsets.push_back(offset);
      }
    }
    calibState = CalibrationState::WaitingForEngineOn;
  } else if (calibState == CalibrationState::WaitingForEngineOn) {
    if (isEngineOn) calibState = CalibrationState::ScanningEngineOn;
    ...
    if (isRevving) calibState = CalibrationState::ScanningRev;
...
const char *VehicleData::GetOffsetSourceName() {
  ...
  case VehicleOffsetSource::IniFallback: return "INI fallback";
  case VehicleOffsetSource::Calibration: return "Calibration (unverified - double check gears/lights!)";
...
bool VehicleData::HasPlausibleLayout(int maxGear) const {
  ...
  const bool gearPlausible = gear <= static_cast<uint8_t>(maxGear + 1) || gear == 0xFF;
  const bool nextGearPlausible = nextGear <= static_cast<uint8_t>(maxGear + 1) || nextGear == 0xFF;
...
bool VehicleData::SetClutch(float clutch) {
  if (!isValid || !std::isfinite(clutch)) return false;
  m_vehicle.SetClutch(clutch);
  return true;
}

bool VehicleData::SetRPM(float rpm) {
  if (!isValid || !std::isfinite(rpm)) return false;
