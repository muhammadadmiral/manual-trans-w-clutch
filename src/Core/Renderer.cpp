...
void DrawTextOverlay(const char *text, float x, float y, float scale, int r,
                     int g, int b, int a) {
  HUD::SET_TEXT_FONT(0);
...
void DrawGearHUD(int manualGear, int maxGear) {
  ...
  if (manualGear == -1) {
    ...
    r = 255;
    g = 80;
    b = 80; // Red for Reverse
  } else if (manualGear == 0) {
    strcpy_s(gearStr, "N");
    r = 255;
    g = 200;
    b = 50; // Gold for Neutral
  } else {
    sprintf_s(gearStr, "%d", manualGear);
    r = 0;
    g = 220;
    b = 255; // Cyan for Forward Gears
  }

  // Draw Big Gear Text
  DrawTextOverlay(gearStr, badgeX + 0.022f, badgeY + 0.012f, 0.95f, r, g, b,
                  255);
}

void DrawGrindWarning() {
  DrawTextOverlay("CLUTCH REQUIRED! (Gears Grinding)", 0.38f, 0.75f, 0.55f, 255,
                  60, 60, 255);
}
